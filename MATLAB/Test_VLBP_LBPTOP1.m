...
[videos y] = size(no_frames_test);

count = 1;
Histogram = cell(1,videos); %where all Histograms will go

for i = 1 %:videos
    
    % Put test_x video of the moment into a matrix
    no_frames = no_frames_test(i,1);
    test_video = test_x(count:count+no_frames-1,:);
    ...
    [frames framelength] = size(test_video);
    
    for j=1:frames
        Imgdat = test_video(j,:); %one row is an entire frame
        Imgdat = reshape(Imgdat,60,80); %reshape into video
