...
nn.learningRateParams.lr             =  0.05; % 0.1; %  learning rate Note: typically needs to be lower when using 'sigm' activation function and non-normalized inputs.
...
disp(nn.epochs);
fprintf('RBM Epochs:');
disp(dbnParams.rbmParams.epochs);
fprintf('LR RBM:');
disp(dbnParams.rbmParams.lrW);
fprintf('LR NN:');
disp(nn.learningRateParams.lr);
fprintf('Momentum:');
disp(nn.momParams.momentum);
fprintf('Dropout:');
disp(nn.dropoutParams.dropoutType);
fprintf('Batchsize:');
disp(nn.batchsize);
fprintf('MaxNormConstraints:');
disp(nn.weightConstraints.maxNormConstraint);
fprintf('L1:');
disp(nn.weightConstraints.weightPenaltyL1);
fprintf('L2:');
disp(nn.weightConstraints.weightPenaltyL2);
fprintf('No Layers:');
disp(nn.noLayers);
fprintf('No Neurons:');
disp(nn.layersSize); %presumably?
